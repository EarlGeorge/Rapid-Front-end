...
(function (root, window, document, factory, undefined) {
  ...
  function initialise(containerSelector, options) {
    const isLicenseValid = options && new RegExp('([\\d\\w]{8}-){3}[\\d\\w]{8}|OPEN-SOURCE-GPLV3-LICENSE').test(options.licenseKey) || document.domain.indexOf('alvarotrigo.com') > -1;
    ...
    function setResponsive(active) {
      const isResponsive = hasClass($body, RESPONSIVE);
    ...
    function setOptionsFromDOM() {
      ...
      if (!options.anchors.length) {
        const anchors = $(`${SECTION_SEL}[data-anchor]`);
        if (anchors.length) {
      ...
      if (!options.navigationTooltips.length) {
        const tooltips = $('[data-tooltip]', container);
        if (tooltips.length) {
    ...
    function prepareDom() {
      ...
      for (let i = 0; i < sections.length; i++) {
        ...
        } else if (options.verticalCentered) {
    ...
    function isDestinyTheStartingSection() {
      const destinationSection = getSectionByAnchor(getAnchorsURL().section);
    ...
    function touchMoveHandler(e) {
      const activeSection = closest(e.target, SECTION_SEL);
      ...
      if (isReallyTouch(e)) {
        ...
        if ($(SLIDES_WRAPPER_SEL, activeSection).length && Math.abs(touchStartX - touchEndX) > (Math.abs(touchStartY - touchEndY))) {
          ...
          if (!slideMoving && Math.abs(touchStartX - touchEndX) > (window.innerWidth / 100 * options.touchSensitivity)) {
            ...
            } else if (isScrollAllowed.m.left) {
    ...
    function fireCallback(eventName, v) {
      ...
      } else if (options[eventName].apply(eventData[0], eventData.slice(1)) === false) {
        return false;
      }
    ...
    function getEventData(eventName, v) {
      ...
      if (!options.v2compatible) {
        ...
        paramsPerEvent = {
          afterRender() { return [container]; },
          onLeave() { return [v.activeSection, v.leavingSection, (v.sectionIndex + 1), v.direction]; },
          afterLoad() { return [v.element, v.anchorLink, (v.sectionIndex + 1)]; },
          afterSlideLoad() { return [v.destiny, v.anchorLink, (v.sectionIndex + 1), v.slideAnchor, v.slideIndex]; },
          onSlideLeave() { return [v.prevSlide, v.anchorLink, (v.sectionIndex + 1), v.prevSlideIndex, v.direction, v.slideIndex]; },
        };
      }
    ...
    function lazyLoad(destiny) {
      ...
      $('img[data-src], img[data-srcset], source[data-src], source[data-srcset], video[data-src], audio[data-src], iframe[data-src]', panel).forEach(function (element) {
        ...
        if (matches(element, 'source')) {
          const typeToPlay = closest(element, 'video') != null ? 'video' : 'audio';
    ...
    function onTab(e) {
      ...
      const activeSection = $(SECTION_ACTIVE_SEL)[0];
      const activeSlide = $(SLIDE_ACTIVE_SEL, activeSection)[0];
      const focusableWrapper = activeSlide || activeSection;
      const focusableElements = $(`${focusableElementsString}:not([tabindex="-1"])`, focusableWrapper);
    ...
    function slideArrowHandler() {
      ...
      } else if (isScrollAllowed.m.right) {
    ...
    function destroy(all) {
      ...
      const clickTouchEvents = [sectionBulletHandler, tooltipTextHandler, slideArrowHandler, slideBulletHandler];
      ['click', 'touchstart'].forEach(function (eventName) {
    ...
    function displayWarnings() {
      ...
      const extensions = ['fadingEffect', 'continuousHorizontal', 'scrollHorizontally', 'interlockedSlides', 'resetSliders', 'responsiveSlides', 'offsetSections', 'dragAndMove', 'scrollOverflowReset', 'parallax'];
    ...
    for (let i = 1; i < arguments.length; i++) {
      ...
      for (const key in obj) {
        if (obj.hasOwnProperty(key)) {
          if (typeof obj[key] === 'object' && obj[key] != null) { out[key] = deepExtend(out[key], obj[key]); } else { out[key] = obj[key]; }
        }
      }
    }
...
if (window.jQuery) {
  (function ($, fullpage) {
    ...
    $.fn.fullpage = function (options) {
      const FP = new fullpage(`#${$(this).attr('id')}`, options);
